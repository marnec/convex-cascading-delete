...
import { exposeApi } from "@00akshatsinha00/convex-cascading-delete";
...
export const addComment = mutation({
  ...
  handler: async (ctx, args) => {
    return await ctx.runMutation(components.convexCascadingDelete.lib.add, {
      text: args.text,
...
export const listComments = query({
  ...
  handler: async (ctx, args) => {
    return await ctx.runQuery(components.convexCascadingDelete.lib.list, {
      targetId: args.targetId,
    });
  },
});
...
export const translateComment = action({
  ...
  handler: async (ctx, args) => {
    return await ctx.runAction(components.convexCascadingDelete.lib.translate, {
      baseUrl: BASE_URL,
...
// the component's API:
export const { list, add, translate } = exposeApi(components.convexCascadingDelete, {
