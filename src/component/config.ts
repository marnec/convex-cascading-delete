...
(2.) Validates configuration structure at definition time
...
basic structural validation. This declarative approach allows developers to specify
their entire deletion graph in one place, making it easy to understand and maintain
the cascade behavior across their application. The function ensures the configuration
 ...
 * it for use with deleteWithCascade operations. It performs basic validation to
 * ensure the configuration is well-formed, though runtime validation of indexes
 * occurs during actual deletion operations.
 * 
 * @param config - Mapping of source tables to their cascade rules
 * @returns Validated cascade configuration
...
export function defineCascadeRules(config: CascadeConfig): CascadeConfig {
  ...
  (3.) Validate that each rule has required 'to' and 'via' properties
  ...
  maps to an array of rules, and that each rule contains the required properties.
  This catches configuration errors early at definition time rather than during
  runtime deletion operations, providing better developer experience and clearer
  error messages.
  */
